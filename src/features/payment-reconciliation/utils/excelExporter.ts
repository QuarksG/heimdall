...
import { applyAuditStyles } from './auditStylePatcher';
...
export class ExcelExporter {
  ...
  public async generateBlob(
    ...
    // Tedarikçi Cari Hareketleri, Disclaimer (always present, always last).
    ...
    const wsVendorLedger = this.vendorLedgerBuilder.createFromComputed(cashierResult);
    XLSX.utils.book_append_sheet(workbook, wsVendorLedger, 'Tedarikçi Cari Hareketleri');

    // 6b. Disclaimer sheet — ALWAYS last, ALWAYS present. Every
    //     data-quality warning from the parse lives here in full; the UI
    //     only announces the count and directs the analyst to this page.
    //     A clean parse writes an explicit "no findings" row so absence of
    //     warnings is stated, never inferred.
    const wsDisclaimer = this.dataQualityBuilder.create(warnings);
    XLSX.utils.book_append_sheet(workbook, wsDisclaimer, DataQualitySheet.SHEET_NAME);
    ...
    const blob = await applyAuditStyles(gridlessBlob, 'Pivot Fatura Türü', auditStyleTargets);
